...
RSpec.describe NetboxClientRuby::DCIM::RearPort, faraday_stub: true do
  subject { sut.new entity_id }

  let(:entity_id) { 1 }
  let(:expected_name) { '1' }
  let(:sut) { described_class }
  let(:base_url) { '/api/dcim/rear-ports/' }

  let(:request_url) { "#{base_url}#{entity_id}/" }
  let(:response) { File.read("spec/fixtures/dcim/rear-port_#{entity_id}.json") }
