...
RSpec.describe NetboxClientRuby::Tenancy::Contacts, faraday_stub: true do
  let(:response) { File.read('spec/fixtures/tenancy/contacts.json') }
  let(:request_url) { '/api/tenancy/contacts/' }
  ...
  describe '#as_array' do
    ...
    it 'returns single instances' do
      subject.to_a.each do |element|
        expect(element).to be_a NetboxClientRuby::Tenancy::Contact
      end
    end
  end
end
