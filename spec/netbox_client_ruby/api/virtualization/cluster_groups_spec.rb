...
RSpec.describe NetboxClientRuby::Virtualization::ClusterGroups, faraday_stub: true do
  let(:expected_length) { 1 }
  let(:singular_type) { NetboxClientRuby::Virtualization::ClusterGroup }

  let(:response) { File.read('spec/fixtures/virtualization/cluster-groups.json') }
  let(:request_url) { '/api/virtualization/cluster-groups.json' }
